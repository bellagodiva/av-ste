...
if DBG:
    ...
    from resnet_causal import CausalResEncoder
    logging.basicConfig(
...
class AVHubertConfig(FairseqDataclass):
    ...
    lookahead_frames: int = field(default=0, metadata={'help': 'number of future frames the transformer can attend to (0=fully causal, N=N-frame lookahead). Each frame = 40ms at 25Hz, so lookahead=2 adds 80ms latency.'})
...
class SubModel(nn.Module):
    def __init__(self, resnet=None, input_dim=None, cfg=None):
        ...
        self.encoder = CausalTransformerEncoder(cfg) if cfg.encoder_layers > 0 else None
...
@register_model("av_hubert_causal", dataclass=AVHubertConfig)
class CausalAVHubertModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: AVHubertConfig,
        ...
        **kwargs
        ...
        resnet = CausalResEncoder(relu_type=cfg.resnet_relu_type, weights=cfg.resnet_weights)
        ...
        self.encoder = CausalTransformerEncoder(cfg)
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        kwargs = {}
        model = CausalAVHubertModel(cfg, task.cfg, task.dictionaries, **kwargs)
        return model
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False,
                         output_layer=None, return_visual=False):
        src_audio, src_video = source['audio'], source['video']
        if mask and self.masking_type == 'input':
        ...
        # features_video: [B, D, T] — clean ResNet output, before audio fusion and transformer.
        # Save a [B, T, D] copy now (before layer_norm / dropout change it) so callers
        # that set return_visual=True get the raw visual signal as prediction target.
        visual_feats_raw = features_video.transpose(1, 2)   # [B, T, D]
        ...
        if return_visual:
            return x, padding_mask, visual_feats_raw
